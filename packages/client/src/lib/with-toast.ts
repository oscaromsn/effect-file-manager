...
type ToastOptions<A, E, Args extends ReadonlyArray<unknown>> = {
  whenLoading:
    | string
    | React.ReactNode
    | ((args: { registry: Registry.Registry; args: Args }) => React.ReactNode | string);
  whenSuccess:
    | string
    | React.ReactNode
    | ((args: { registry: Registry.Registry; result: A; args: Args }) => React.ReactNode | string);
  whenFailure:
    | string
    | React.ReactNode
    | ((args: {
...
export const withToast =
  <A, E, Args extends ReadonlyArray<unknown>, R>(options: ToastOptions<A, E, Args>) =>
          ...
          Effect.sync(() => {
            const message =
              typeof options.whenFailure === "function"
                ...
                : options.whenFailure;
            if (Option.isOption(message) && message._tag === "None") return toast.dismiss(toastId);
